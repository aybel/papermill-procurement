...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <a href="{{ env('FRONTEND_URL', 'http://localhost:3000') }}/set-password?token={{ $token }}&email={{ urlencode($user->email) }}" class="button">Establecer Contraseña</a>
            <p>Si no puedes hacer clic en el botón, copia y pega la siguiente URL en tu navegador:</p>
            <p>{{ env('FRONTEND_URL', 'http://localhost:3000') }}/set-password?token={{ $token }}&email={{ urlencode($user->email) }}</p>
